...
-- Dev Plantilla TipTap chip/attr-mustache QA fixture (PR1) + edited shape preservation (PR2).
...
-- QA: visual ↔ HTML, add sibling after img/table, save; Sources → Execute foto; Template → Render.
-- Expect: src/href mustaches stay attributes; no chip scaffolding; comments and else-if intact;
--         div stays div; bare table cells stay bare (no colgroup/min-width); authored link has no target/rel.
-- Editor: broken img / relative iframe are expected until preview resolves {{foto.url}} (Source alias foto).
