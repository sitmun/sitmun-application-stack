...
-- Oracle final-state mirror for TipTap chip/attr + media placeholders + edited shape QA fixture.
-- Open: /#/taskTemplate/9030/15
-- Visual: mustache img/iframe show binding placeholders; select for src/alt/title inspector.
-- Preview: Sources → Execute foto, then Template → Render (links open in a new tab).
-- Latest templateHtml applied by changeset sitmun:69-dev-plantilla-tiptap-chip-fixes-qa-media-ux-oracle.
...
--changeset sitmun:69-dev-plantilla-tiptap-chip-fixes-qa-media-ux-oracle context:dev dbms:oracle
--preconditions onFail:MARK_RAN
--precondition-sql-check expectedResult:1 SELECT COUNT(*) FROM STM_TASK WHERE TAS_ID = 9030
UPDATE STM_TASK
